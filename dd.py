...
{H2}
 {H2}
         Fucked By DANAR {P2}Coder""",width=80,style=f"{color_panel}"))
def info():
	prints(f"""{B2}{B2}
{B2} {P2}OWNER➠DANAR {B2}     {B2} {P2}{B2} 
{B2}{B2}""")
print('DANAR UP')                       
def banner():
	clear()
...
def dump_publik():
	...
	try:
		...
		print('▄︻┻═┳一 Internetmu Gak Ada Bodoh')
		exit()
	except (KeyError,IOError):
		print('▄︻┻═┳一 Pertemanan Tidak Publick Cookie And Token Anda Busuk')
		exit()
...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		kl = input('▄︻デC̷H̷A̷N̷D̷ ̷I̷D̷ ̷D̷A̷W̷E̷══━一 '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
{B2}""")
	hu = input('▄︻デD̷A̷N̷A̷E̷A̷K̷ ̷D̷A̷N̷A̷══━一➣ ')
	if hu in ['1','old']:
...
def su():
	...
	ch = input('▄︻デD̷A̷N̷A̷E̷A̷K̷ ̷D̷A̷N̷A̷══━一➣ ')
	if ch in ['1','01']:
		mie()
	elif ch in ['2','02']:
		sulap()
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'\r[deep_white] ▄︻デD̷A̷N̷A̷R̷═━一 {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ses = requests.Session()
...
def crackfree(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white] RS7->DANAR {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def getkey():
    ...
    try:
        ...
        none();time.sleep(1)
        print("")
        print("\033[0m╰─ Author: DANAR-CODER ")
        print("\033[0m╰─ Telegram: RS7_TEAM ");time.sleep(2)
        print ("")
...
Author = 'DYNO-DECODER'
Version = 5.1
Facebook = 'Facebook.com/DANARCRACKER'
Instagram = 'Instagram.com/DANAR'
